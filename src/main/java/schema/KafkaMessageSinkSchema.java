...
public class KafkaMessageSinkSchema implements KafkaRecordSerializationSchema<GroupedData> {
    ...
    public KafkaMessageSinkSchema(String topic) {
        this.topic = topic;
    }

    @Override
    public ProducerRecord<byte[], byte[]> serialize(
    		GroupedData message, KafkaSinkContext context, Long timestamp) {
        try {
            return new ProducerRecord<>(
                    topic,
                    null, // choosing not to specify the partition
                    new Date().getTime(),
                    String.valueOf(message.key).getBytes(),
                    objectMapper.writeValueAsBytes(message));
        } catch (JsonProcessingException e) {
            throw new IllegalArgumentException(
                    "Could not serialize record: " + message, e);
        }
